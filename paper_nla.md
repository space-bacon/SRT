...
![**Figure 12. The Lancaster pitchfork, read inside the decoding manifold.** The supercritical pitchfork $\dot{x}=\mu x - x^{3}$ is the canonical normal form Lancaster (2025) uses to model interpretant divergence under polarising pressure. Under the *standard* reading (left), $\mu$ is community polarisation pressure on a discourse and the two branches are competing contested readings of a single sign. Under the *decoding-manifold* reading proposed here (this paper, §12), $\mu$ is best-of-$K$ search budget (or any utility-based selection pressure on a candidate pool), the symmetric below-threshold branch is the greedy/modal $\hat{x}$ in its native basin at centred fve $\approx\!0.59$, and the upper post-threshold branch is the paraphrase-manifold $\hat{x}$ at centred fve $\approx\!0.80$, which is $\rho_{\text{cen}}\!\approx\!0.99$ of the paraphrase ceiling. (Both branches are quoted here in the same unit; an earlier caption mixed centred fve for the low branch with $\rho_{\text{cen}}$ for the high one.) Logp is a within-basin quantity and cannot move probability mass across the separatrix; centred cosine to the target $v$ is the order parameter that does. This figure is a reading, not a measurement: a pitchfork predicts a bimodal per-target distribution with a mixture weight that moves with $K$. We pre-registered that test and ran it on a fresh 2400-target sample with the canonical checkpoint (M = 200, K = 64, means reproducing the banked run). The per-target distribution of best-of-$K$ centred fve is unimodal at every $K$ from 1 to 64 (Hartigan dip p = 0.74 to 0.96), with a spread that barely moves (0.089 to 0.102) while the whole population slides rightward together. There is no greedy branch and no paraphrase-manifold branch; there is one population whose max improves with $K$, as a max over exchangeable samples must. The greedy gap, the failure of logp to rank candidates and the success of anchor cosine are all real, but they do not require a bifurcation and the distribution does not show one. The reading stays in this figure as the hypothesis it was, marked as tested and not supported on this backbone; details and artifacts in `paper_format_susceptibility.md`. The Stage-2 validation that the same normal form fits political-polarisation data (Pearson $r\!=\!0.884$ on five Supabase communities, 19K articles, 141K Peircean sign annotations; Lancaster, 2026a) is what made the analogy worth testing.](artifacts/explainers/15_lancaster_pitchfork.png)
