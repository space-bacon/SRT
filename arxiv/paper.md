...
header-includes:
  - \usepackage{etoolbox}
  - \AtBeginEnvironment{abstract}{\footnotesize\linespread{1.0}\selectfont\setlength{\parskip}{3pt}\setlength{\parindent}{0pt}}
---
...
5. **Cross-backbone substrate evidence and interiority dissection.** A raw-hidden 1-NN probe (§5.12) demonstrates that the discourse-community signal v8a's CDH amplifies is latent in `Qwen/Qwen3-8B` and `mistralai/Mistral-7B-v0.3` at comparable strength to Qwen 2.5-7B's own raw signal, supporting the backbone-agnostic claim without requiring an adapter re-train. An eight-probe interiority study on the v8a checkpoint (§5.13) maps how the four-module decomposition acquires functionally distinct channels: a layer-as-organ map by regime, a 1-NN community probe at $7.7\times$ chance, a BOS-register length-scaling probe (amplitude stable across $8\times$ length sweep, peak-to-peak $\leq 4.8\%$), a per-channel-per-regime BOS-sink ablation showing the inject channels function as register slots while the regime classifier reads content-borne signal, and a within-prompt-trajectory analysis showing the regime classifier holds steady-state regime-specific posteriors that mean pooling collapses.

6. **Interpretability is compatible with sentence-embedding quality, and the cheapest meaningful gain on this surface is parameter-space interpolation between two specialised optima.** Holding the v8a interpretability scaffold (BEN, MAH, RRM, prototype-free CDH) frozen at evaluation, light downstream training of the encoder output channel on standard pair data raises mean MTEB-STS Spearman over 40 splits from $0.210$ (v8a) to $0.3634$ (v15a, shipped as `RiverRider/srt-adapter-v1.0`) under InfoNCE on NLI alone, then to $0.3707$ (v18) under CoSENT on $5{,}000$ STSB pairs. Linear weight-averaging of v18 (English-purist) and v21a (multilingual-leaning, teacher-distilled with mxbai cosines) at $\alpha = 0.5$ yields v22c\_a050 at mean STS $0.3744$ ($24/40$ wins over v18) at *zero additional training cost*. The interpretability readouts of §§5.4–5.5 and §5.13 remain stable across the entire v12–v22c\_a050 lineage. Two methodological consequences follow: when the eval metric is Spearman, use a rank-aligned loss (CoSENT) rather than absolute-value regression on cosines (MSE); and when two specialised adapters exist, weight-averaging is the cheapest first move before retraining (Section 5.14).
...
Each MAH layer reads hidden states from a backbone layer and computes a divergence vector at each position quantifying where meaning forks under contextual interpretation. (As a measurement claim this is the layer's *design* target; \u00a76.9 reports a direct separatrix-probe test that constrains the stronger semantic reading and supports a more conservative gloss in terms of community-conditioned local prediction surprise.)
...
### 4.1 Data (semiotic probe suite, v5–v8a)
...
**Scope of this corpus.** The Reddit corpus described above is the *only* training data used for the semiotic probe suite (v5–v8a) reported in §§5.1–5.13. The downstream MTEB-STS lineage (v12–v22c\_a050, §5.14) does *not* train on Reddit. It warm-starts from the v8a checkpoint, freezes the BEN, MAH, RRM, and CDH heads, and trains only the encoder output channel on standard sentence-pair benchmarks (NLI, STSB, Quora, optionally teacher-distilled with `mixedbread-ai/mxbai-embed-large-v1`). Per-version corpus, size, loss, and licensing are tabulated inside §5.14; no Reddit data is used past v8a. This separation is what allows the interpretability readouts of §§5.4–5.5 and §5.13 to remain stable across the entire v12–v22c\_a050 arc: only the encoder channel retrains.
...
### 5.5 Regime calibration (v5; replicated on v8a)
...
| Metric | v5, step 17K | **v8a, step 10K** |
|---|---|---|
| AUROC | 0.9899 | **0.9899** |
| Brier score | 0.0102 | **0.0103** |
| **ECE (15 bins)** | **0.0009** | **0.00091** |
| Max bin gap | 0.054 (in $[0.20, 0.27]$, $n = 465$) | 0.052 (mid-range bin) |
| Bins on diagonal (within $\pm 2\sigma$) | 15 / 15 | **15 / 15** |

The model is exceptionally well-calibrated: ECE of $9 \times 10^{-4}$ on 351K tokens, with the largest bin gap at 0.054 in the very-low-density mid-range. The reliability diagram (Figure 5.1, `artifacts/regime_calibration/v5_step17000.png`; v8a replication at `artifacts/regime_calibration/v8a_step10000.json` and rendered as Fig. 9 of the public demo card) traces the diagonal almost perfectly across all 15 bins. The v8a replication confirms that removing the prototype bottleneck (§5.9) did not degrade calibration: the regime head still produces directly-thresholdable probabilities, with no need for post-hoc Platt or isotonic correction. This unblocks downstream use of $P(\text{supercritical})$ as a probability rather than a relative score.
...
### 5.12 Cross-backbone signal probe (Qwen 2.5-7B / Qwen 3-8B / Mistral-7B-v0.3)

The single-backbone limitation flagged in §6.6 is partially addressable without re-training the adapter on a new backbone. The first question to ask is whether the discourse-community signal that v8a's CDH amplifies is *specific to Qwen 2.5*, or whether it is latent in other 7B-class causal LMs and merely needs to be read out. We answer with the simplest possible probe: take raw mean-pooled hidden states from each backbone on `data/val_200.jsonl` (200 samples, 35 coarse Reddit communities), then compute leave-one-out 1-nearest-neighbor recall@1 on cosine similarity. No training, no labels at inference, no adapter.

| Backbone | best raw-hidden recall@1 | best layer | $\times$ chance |
|---|---:|---:|---:|
| `Qwen/Qwen2.5-7B`        | 0.260 | 16 | 9.1$\times$ |
| `Qwen/Qwen3-8B`          | 0.220 |  4 | 7.7$\times$ |
| `mistralai/Mistral-7B-v0.3` | **0.295** | 16 | **10.3$\times$** |

For reference, chance on this 35-class probe is $1/35 = 0.0286$, and the v8a SRT-Adapter on `Qwen/Qwen2.5-7B` reaches recall@1 $= 0.484$ ($16.7\times$ chance) on the larger Reddit validation split (§5.9). Three observations follow.

*The signal is not Qwen-specific.* `Mistral-7B-v0.3` raw hidden states actually carry slightly more discourse-community information than `Qwen2.5-7B` raw hidden states do (10.3$\times$ vs 9.1$\times$ chance on the same probe). The community-discriminative subspace that the adapter's CDH learns to amplify is present in both Qwen-family and Mistral-family backbones at comparable strength. Whatever the adapter is reading off the residual stream, the *raw substrate* it depends on is a property of contemporary 7B-class causal LMs, not of one specific tokenizer or training recipe.

*The adapter's contribution is a \~$1.8\times$ amplification of that substrate.* v8a on Qwen 2.5-7B delivers $16.7\times$ chance, which is roughly $1.8\times$ the strongest raw-hidden-state baseline in the table (Mistral, $10.3\times$) and $1.8\times$ the same backbone's own raw signal at the best layer ($9.1\times$). The adapter is doing real discriminative work on top of what the frozen backbone already encodes; the residual stream alone gets you most of the way to chance-level retrieval but does not reach the v8a number on its own.

*The architecture should transfer.* The CDH consumes a single hidden-state tensor at `community_layer_idx`; nothing in the adapter is Qwen-tokenizer-specific. Re-training the v8a recipe on Mistral-7B-v0.3 is a plausible future-work item; the raw-substrate result above is the strongest zero-cost evidence we currently have for the cross-backbone hypothesis. The full reproduction script is `scripts/cross_backbone_probe.py`, JSON results are at `artifacts/cross_backbone.json` and the two extra-backbone files (`cross_backbone_extra.json`, `cross_backbone_mistral.json`), and the rendered visualization is Fig. 8 of the public demo card.

### 5.13 Interiority study: post-release diagnostics on v8a

This subsection summarizes a sequence of post-release probes on the v8a checkpoint that dissect *how* the adapter encodes semiotic regime, with full numbers and reproduction commands in [`docs/INTERIORITY_V1_FINDINGS.md`](https://github.com/space-bacon/SRT/blob/main/docs/INTERIORITY_V1_FINDINGS.md). The probe battery is 11 regimes (`metaphor`, `counterfactual`, `negation_modality`, `irony`, `literal`, `refusal_bait`, `lyric`, `deixis`, `self_reference`, `quoted_speech`, `code`) $\times$ 25 prompts. The community vector throughout is the 64-D CDH output (`use_prototypes=False`).

*Layer-as-organ map.* Per-regime z-score winners across nine adapter channels show channel specialization: `code` peaks on $\hat{r}$, regime entropy, and $\text{div}\,L21$; `metaphor` peaks on $\text{inj}\,L14$, $\text{inj}\,L21$, and the chain residual; `literal` peaks on $P(\text{super})$. The four-module decomposition acquires functionally distinct channels under v8a's continuous architecture.

*Community 1-NN.* Leave-one-out 1-NN top-1 accuracy on the 11-regime probe battery is $0.70$, with bootstrap mean $0.77$ and CI95 $[0.72, 0.82]$ over 2000 iterations. This is $7.7\times$ chance on a 1/11 task. The top centroid pair `code`$\leftrightarrow$`literal` (Euclidean $d = 1.07$) stays the most-separated pair in $85.5\%$ of bootstrap iterations; the bottom pair `quoted_speech`$\leftrightarrow$`self_reference` ($d = 0.39$) stays last in $83.0\%$. The point estimate of community separation ratio is $0.85$, which we treat as a lower bound: the bootstrap CI95 $[0.86, 0.98]$ is upward-biased by within-cluster duplicate sampling.

*BOS-register length scaling.* A per-token trajectory probe across $T \in \{16, 49, 79, 129\}$ token windows (4 length bins $\times$ 5 prompts $\times$ 11 regimes) shows that the adapter's write channels ($\text{inj}\,L14$, $\text{inj}\,L21$, $\text{div}\,L7/L14/L21$, chain residual) form a *fixed-size BOS register*: BOS-token amplitude on those channels is stable across an $8\times$ length sweep, staying within $\pm 2.5\%$ of the $T \approx 16$ baseline (peak-to-peak $\leq 4.8\%$). v8a learns to compress each prompt's regime structure into a single fixed-amplitude write at the BOS slot; mid-prompt activity on inject channels is small.

*BOS-sink ablation, per channel $\times$ regime.* The above is consistent with two readings: either the adapter encodes its global regime summary at BOS and only at BOS, or BOS is a sink that contributes to but does not solely determine the regime ranking. We disambiguate by computing, for each channel $c$ and regime $g$, the z-score of $g$'s mean activation against the other ten regimes twice: once with BOS included in the pooling window ($z^{\text{with}}_{c,g}$), once with BOS excluded ($z^{\text{no}}_{c,g}$), and reporting $\Delta |z|_{c,g} = |z^{\text{with}}_{c,g}| - |z^{\text{no}}_{c,g}|$. On $\hat{r}$ and on regime entropy, $|\Delta|z|| \leq 0.27$ for every regime: the regime-classifier scalars read essentially the same regime structure with or without the BOS slot, so the regime signature is *content-borne*. On $\text{inj}\,L14$ the deltas are large and signed: `metaphor` $\Delta = -0.64$ (regime signal is BOS-localized; removing BOS *strengthens* the relative z-score of the content-token mean), `literal` $\Delta = +0.98$ (regime signal is *anti*-localized to BOS; removing BOS dilutes the regime signature). The chain-residual channel is intermediate (max $|\Delta|$ $= 0.58$ on `quoted_speech`). The honest synthesis is that the inject channels are functioning as register slots that hold a global per-prompt summary, while the regime classifier reads regime structure off content tokens directly. A simpler aggregate measure, regime-rank preservation under BOS exclusion, holds in 9/9 channels with mean Spearman $\rho = +0.74$, which is consistent with but coarser than the per-channel per-regime breakdown above.

*Within-prompt trajectories.* Plotting per-position curves for the regime-classifier scalars ($\hat{r}$, regime entropy) across 19 content position bins (BOS dropped) per regime shows that the temporal structure hidden by mean pooling is informative. $\hat{r}$ separates regimes by sustained amplitude: `code` sits at $\hat{r} \geq 0.85$ across the full window; `negation_modality`, `deixis`, and `refusal_bait` sit at $\hat{r} \leq 0.40$; the other seven regimes occupy a middle band $0.45$–$0.75$. Regime entropy puts `code` in a sustained $0.18$–$0.32$ band while every other regime stays at $\leq 0.05$ across all 19 content positions. The classifier is consistently *uncertain* about the regime label whenever the prompt is `code` and consistently confident on every other regime, and that uncertainty is steady-state across positions, not a transient. Mean-pooled scalars collapse this to a single number per prompt and lose the structure.

*Architectural reading of the interiority study.* v8a learned to compress each prompt into a fixed-size *regime fingerprint* written into the BOS register on inject channels, while the regime classifier itself operates as a content-distributed channel that reads regime evidence off non-BOS tokens. This is consistent with §6.3: the inject-back arm carries no measurable signal in v8a, so the adapter's content-routing capacity is under-used. v9 (Section 7) targets this directly with a coverage-loss term that penalizes register concentration. The interiority probes do not change v8a's headline numbers; they explain *where* in the architecture those numbers come from. Visualizations (per-channel z-score heatmaps, per-regime within-prompt trajectories, BOS-ablation $\Delta |z|$ matrix) appear as Figs. 10 and 11 of the public demo card.

### 5.14 Sentence embedding quality: from interpretability to MTEB-STS (v12 → v22c)

The v5–v8 arc established that a 12.7M-parameter adapter on a frozen Qwen 2.5-7B can carry community, divergence, and regime structure without degrading cross-entropy. It did not establish that the same architecture, with light downstream training of the community channel, is competitive as a *sentence embedding* model. v8a's mean MTEB-STS Spearman across 40 splits was $0.210$, dominated by the fact that the community vector was optimised for unsupervised Reddit retrieval rather than human STS judgments. The v12–v22c lineage answers the embedding question directly: holding the v8a interpretability scaffold frozen at evaluation, can the encoder output be trained on standard STS-style pair data without losing the diagnostic readouts of §§5.1–5.13?

*v12 — first leaderboard-credible STS.* Warm-starting from v8a, one InfoNCE epoch on a 396K mixed pair corpus (NLI + Quora + MS MARCO triples) raises mean STS Spearman to $0.346$ (+$0.136$ absolute, $+64\%$ relative on a single A6000 in $\sim 100$ minutes). The top splits are HUME-SICK-R $0.792$, STS17 split 2 $0.723$, SemRel24STS $0.714$. Per-split gains are dominated by the English subset (mean $17$ splits $0.574$) with cross-lingual splits remaining flat. The interpretability readouts (regime ECE, $\hat r$ distribution, hallucination AUROC) are unchanged within seed variance: the InfoNCE objective acts on the encoder output channel only and does not perturb the BEN, MAH, or RRM heads.

*v15a — NLI-only, official v1.0 release.* Removing MS MARCO from the v12 corpus and training one epoch of InfoNCE on the NLI subset alone (396K pairs reduced to NLI's $\sim 280$K) raises mean STS to $0.3634$ (+$0.017$ over v12). MS MARCO, which is a retrieval corpus and not an STS corpus, was actively hurting the embedding objective. v15a is the checkpoint shipped as `RiverRider/srt-adapter-v1.0` (§5.15). Headline English numbers: HUMESICK-R $0.7973$, STSBenchmark $0.4434$, SICK-R $0.6072$, STS15 $0.6008$. The Cap-A community-NMI benchmark (§5.2 protocol, evaluated on Banking77 intent labels rather than Reddit subreddits) is $0.5391 \pm 0.008$, up from v8a's $0.4427$ ($+0.096$, $\approx 13\sigma$). Hallucination AUROC remains $0.576$, regime distribution $96.9\%$ supercritical on Reddit val. v15a is the first checkpoint that satisfies both the embedding-quality criterion and the interpretability-preservation criterion simultaneously.

*v17 — cosine-MSE on STSB regresses (negative result).* Adding $5{,}000$ STSB sentence pairs and training a regression head with MSE loss on the absolute cosine score drops mean STS to $0.3531$ ($-0.010$ vs.\ v15a). Dev Spearman peaks at $0.6936$ in-distribution and falls to $0.5613$ on test, a $0.13$ generalisation gap. The lesson, made explicit by v18, is that MSE on absolute cosine is Pearson-aligned (it minimises squared error against the *value* of the human label) while MTEB scores Spearman (which depends only on the *rank order*); training under MSE optimises the wrong correlate.

*v18 — CoSENT pairwise loss, first SOTA.* Replacing the MSE head with the CoSENT pairwise rank loss (Su, 2022; scale $= 20$, lr $= 1\times 10^{-5}$, 5 epochs $= 565$ steps on the same $5{,}000$ STSB pairs) yields mean STS $0.3707$, $+0.0074$ over v15a, winning on $29/40$ splits. The same data that regressed under MSE generalises and improves under CoSENT, including on cross-lingual STS17 subsets (en-fr $+0.040$, en-ar $+0.037$, BIOSSES $+0.013$) despite English-only training. Best-checkpoint saver caught a peak at step $100$ (dev Spearman $0.6856$); the schedule then over-trained, motivating v19a's shorter run. The headline architectural finding from v17 vs.\ v18 is that *loss-function choice swamps data-recipe choice* on this surface: the same $5{,}000$ pairs move mean STS $-0.010$ or $+0.007$ depending on whether the loss is rank-aligned with the eval metric.

*v19a/v19b, v20 — schedule and label ablations (negative).* v19a (lr $= 5 \times 10^{-6}$, 2 epochs, val\_every $= 25$) matches v15a on mean STS within seed variance and does not beat v18; the over-training in v18 cost $\leq 0.001$. v19b (CoSENT on NLI-as-pairs $+$ Quora paraphrases, $\sim 207$K pairs) reaches $0.3709$, nominally a $+0.0002$ improvement that lies inside the seed-variance band reported below. v20 (NLI categorical labels treated as graded scores: entail $= 1.0$, neutral $= 0.5$, contradict $= 0.0$, trained under CoSENT on $200$K pairs) drops mean STS to $0.3667$ ($-0.004$). The graded-NLI label scheme warps the calibration of the rank-loss across the full label range and damages English STS (HUMESTSBenchmark $-0.117$, STS13/14 $-0.02$ to $-0.03$) while modestly improving cross-lingual splits. Categorical NLI labels are not a faithful proxy for graded similarity even under a rank-aligned loss.

*v21a — teacher distillation with continuous similarity labels.* Re-scoring the v20 pair pool ($200$K NLI premise/hypothesis pairs $+$ $7$K STSB pairs) using \texttt{mixedbread-ai/mxbai-embed-large-v1} as a teacher and treating the teacher cosine as the continuous CoSENT label (otherwise identical hyperparameters to v18) yields mean STS $0.3720$ ($+0.0012$ over v18, $14/40$ wins). The gains are concentrated on cross-lingual splits (IndicCrosslingualSTS en-bn $+0.226$, en-tr $+0.161$, en-ta $+0.151$, en-te $+0.146$); English splits regress (HUMESTSBenchmark $-0.111$, STSBenchmark $-0.084$, STS13/14 $-0.07$ to $-0.09$). The diverse pair pool *is* informative when scored against a continuous similarity teacher; the v20 problem was the categorical labels, not the pool. v21a establishes a multilingual-leaning sibling to v18's English-purist baseline.

*v21b, v22a, v22b — small wins and a near-saturation diagnostic.* v21b\_a050 (linear weight average of v18 and v20 at $\alpha = 0.50$) reaches $0.3709$, recovering v20's multilingual gains at the cost of marginal English regressions; v21b\_a070 ($\alpha = 0.70$) reaches $0.3738$ ($+0.0031$ vs.\ v18) and is the most balanced of the v18+v20 souping configurations but inferior to v22c below. v22a (STSB upsampled $5\times$ to increase the graded-pair fraction) reaches $0.3712$ ($+0.0005$). v22b (multi-teacher distillation, mxbai $+$ \texttt{intfloat/multilingual-e5-large}, averaged cosines as labels) reaches $0.3708$ ($+0.0001$). Each of these results is at or below the seed-variance floor.

*v22c\_a050 — model souping yields current SOTA.* Linear-averaging the v18 and v21a adapter weights at $\alpha = 0.50$ (i.e., $\theta_{\text{v22c}} = 0.5\,\theta_{\text{v18}} + 0.5\,\theta_{\text{v21a}}$) yields mean STS $0.3744$, the highest measured value on this lineage, winning on $24/40$ splits over v18 with *zero additional training*. The English splits hold (HUMESICK-R $0.7682$, $-0.033$ vs.\ v18, within seed variance), the multilingual gains transfer (STS17 zh-en $0.6647$, $+0.143$ vs.\ v18; IndicCrosslingual mean stabilises at $-0.021$ vs.\ v21a's $+0.05$ to $+0.22$ wins-with-English-cost), and the IndicCrosslingual losses against v21a's standalone configuration are recovered. The procedural cost is two matrix additions and a checkpoint save. We read this as direct evidence that v18 and v21a sit in a connected loss basin despite their corpus-level specialisation: the encoder's parameter manifold under CoSENT is locally convex enough that linear interpolation between two specialised optima recovers a strictly better Pareto point. This is the cheapest meaningful improvement the lineage has produced.

*Seed variance.* v18 and v22c\_a050 were each re-evaluated three times on the full 40-split MTEB STS suite under bfloat16 with DataLoader nondeterminism (seeds $0, 1, 2$). Mean-STS variability was $\pm 0.001$ to $\pm 0.003$ per seed across both checkpoints. We treat per-task deltas $\leq 0.002$ as within noise.

*Beyond-STS scope (v22c\_a050\_beyond).* On a curated MTEB subset spanning Classification (Banking77, EmotionClassification, MTOPDomainClassification), Retrieval (SciFact, NFCorpus, ArguAna), Reranking (SciDocsRR, AskUbuntuDupQuestions), and PairClassification (SprintDuplicateQuestions), v22c\_a050 retains the v15a-era profile: strong on intent-style classification (Cap-A NMI $\approx 0.54$), weak on retrieval (SciFact nDCG@10 $\approx 0.03$, NFCorpus $\approx 0.01$). STS-only training does not transfer to retrieval, as expected; the adapter is a sentence-similarity model, not a general-purpose encoder. This is an honest scope statement, not a regression.

*Summary table.* Mean MTEB-STS Spearman over $40$ splits, all warm-started from v8a unless noted; all eval at fp32 on a single A6000 except where noted as bfloat16 seed re-runs.

| Variant | Mean STS | $\Delta$ vs.\ v15a | Recipe | Status |
|---|---|---|---|---|
| v8a (baseline) | $0.210$ | $-0.153$ | Reddit unsupervised, no STS training | Interpretability baseline |
| v12 | $0.346$ | $-0.017$ | InfoNCE on NLI $+$ Quora $+$ MS MARCO ($396$K) | First STS-credible |
| **v15a** ($=$ **v1.0**) | $0.3634$ | $-$ | InfoNCE on NLI only ($\sim 280$K) | Official release |
| v17 | $0.3531$ | $-0.010$ | $+\,5$K STSB, MSE on cosine | Negative (Pearson $\neq$ Spearman) |
| **v18** | $0.3707$ | $+0.0074$ | $+\,5$K STSB, CoSENT pairwise | SOTA \#1 |
| v19a | $\sim 0.3638$ | $-0.0001$ | v18 with shorter schedule | Tie |
| v19b | $0.3709$ | $+0.0075$ | CoSENT on NLI-as-pairs $+$ Quora ($\sim 207$K) | Tie within variance |
| v20 | $0.3667$ | $+0.0033$ | CoSENT on graded-NLI labels | Negative on English |
| **v21a** | $0.3720$ | $+0.0086$ | CoSENT on mxbai-distilled $200$K pairs | SOTA \#2 (multilingual-leaning) |
| v21b\_a050 / a070 | $0.3709$ / $0.3738$ | $+0.008$ / $+0.010$ | Soup v18 $+$ v20 ($\alpha = 0.5/0.7$) | Tie / minor win |
| v22a | $0.3712$ | $+0.0078$ | STSB $5\times$ upsample | Tie within variance |
| v22b | $0.3708$ | $+0.0074$ | Multi-teacher (mxbai $+$ e5) | Tie within variance |
| **v22c\_a050** | $0.3744$ | $+0.011$ | Soup v18 $+$ v21a ($\alpha = 0.5$), zero training | **SOTA (current)** |

*Architectural reading.* The v8a interpretability scaffold (frozen at the BEN, MAH, RRM, and prototype-free CDH from §§5.9 and 5.13) is compatible with competitive sentence embedding once the encoder output channel is tuned with a rank-aligned loss on standard pair data. The relative ordering of variants is determined by loss-function choice (v17 vs.\ v18) and label-scheme choice (v20 vs.\ v21a) far more than by architecture, and the cheapest meaningful win on this surface is *parameter-space interpolation between two specialised optima* (v22c\_a050) rather than further data-recipe iteration. This is consistent with the §6.5/§6.8 reading that the community vector and $\hat r$ measure orthogonal axes (alignment $\times$ activity): tuning the alignment axis on STS pair data does not perturb the activity axis the BEN reads, which is why the interpretability readouts of §§5.4–5.5 and §5.13 are stable across the entire v12–v22c\_a050 lineage.

### 5.15 Public release: srt-adapter-v1.0

The v15a checkpoint is shipped publicly as `RiverRider/srt-adapter-v1.0` on the Hugging Face Hub, accompanied by an inference-only demonstration Space at `RiverRider/srt-adapter-v1.0-demo` and the prior-generation v8a release at `RiverRider/srt-adapter-v8a` / `RiverRider/srt-adapter-v8a-demo`. The released artifacts are: `adapter.safetensors` ($12.7$M parameters, the four trainable submodules only), `config.json` (the `SRTConfig` dataclass serialised), the training and benchmark logs, and a model card documenting the scope statement above (strong English STS and intent clustering; weak retrieval; mediocre cross-lingual). The full source for the adapter, the training script, and the evaluation harness is available at `https://github.com/space-bacon/SRT` under the Apache 2.0 license; the released weights inherit the same license, while inference requires the `Qwen/Qwen2.5-7B` backbone under its own Qwen license. The v22c\_a050 model-soup checkpoint of §5.14 is shipped separately as `RiverRider/srt-adapter-v22c_a050` and supersedes v15a on STS quality while preserving the same interpretability surface; v15a is retained as the canonical v1.0 release for reproducibility against the Cap-A and §5.4 hallucination benchmarks.
...
The empirical situation through v8b is that this pressure has resolved on the *neutral* side of the helpful/neutral boundary: ablating the inject-back arm at evaluation produces no measurable downstream change on CE, on the regime classifier, or on the hallucination probes (Sections 2.5, 5.7, 5.9). The safety valve held, but the channel through which the meta-state would modulate generation has not learned to carry signal. The interiority study in §5.13 sharpens this picture: the inject channels *do* carry signal, but it is concentrated in a fixed-size BOS register (BOS amplitude is stable across an $8\times$ length sweep, peak-to-peak $\leq 4.8\%$), with the per-channel $\times$ per-regime BOS-ablation analysis showing that some regimes (e.g. `metaphor`) localize their inject signature at BOS and others (e.g. `literal`) are anti-localized to BOS. The arm has learned to *summarize* but not to *modulate*. v9's coverage-loss design (Section 7, Appendix A version history) is directed at exactly this: penalize register concentration and credit distributed mid-prompt activity, so that the inject channel has gradient pressure to behave as a per-token modulator rather than a per-prompt summary slot.
...
This revision of the architectural story is cleaner, not weaker: the model has two distinct outputs that measure two distinct things, and the data tells us which is which. A direct test of the activity/alignment decomposition on a hand-authored separatrix battery is reported in §6.9; that test confirms both axes in the predicted directions and, in the same sweep, falsifies the earlier reading of MAH as an "interpretive divergence" channel.
...
5. **Single backbone (partially answered).** Adapter-trained results are reported only for Qwen 2.5-7B. The cross-backbone raw-hidden probe in §5.12 shows the discourse-community signal that v8a amplifies is present at comparable strength in `Qwen/Qwen3-8B` ($7.7\times$ chance on raw hidden states) and `mistralai/Mistral-7B-v0.3` ($10.3\times$ chance, slightly stronger than Qwen 2.5-7B's own $9.1\times$). This is consistent with the architecture being backbone-agnostic and with the adapter's $\sim 1.8\times$ amplification factor transferring, but the actual adapter-on-Mistral re-train has not yet been executed. The full backbone-agnostic claim still requires that re-training, which is a v9-onward work item.
...
### 6.9 Direct test on a separatrix battery: activity/alignment confirmed, MAH reading constrained

The Haylett separatrix-illusion construct is operationalisable as a direct test of the v8a readouts. We hand-authored a 61-item battery (`data/probes/separatrix_illusion_v1.jsonl`) covering load-bearing physics-and-mysticism flashpoints (entanglement, field, resonance, holism, emergence, attractor, and similar terms). Each item has a neutral prompt plus three scripted continuations: a *technical* one that stays inside the disciplinary register, a *mystical* one that drifts onto the geometrically adjacent but epistemically disjoint basin, and a *bedrock* one that stays inside the technical basin while making a coordinating philosophical move (in the Wittgensteinian / Quinean / Cavellian sense). Three predictions follow from the §6.5 activity/alignment decomposition together with the original §3.3 reading of MAH:

- (P1) the *community head* should place the technical and bedrock branches closer together than the technical and mystical branches in 64-D community-vector space;
- (P3) the *BEN* $\hat r$ should be elevated on the prompts themselves, regardless of which continuation is later attached, since the prompts are by construction interpretively dense;
- (P4) the *MAH* peak divergence should be larger on (technical-prompt + mystical-continuation) than on (technical-prompt + bedrock-continuation), if MAH is reading register collision in the way §3.3 originally claims.

We ran v8a end-to-end on a frozen Qwen2.5-7B (244 forwards, 13 s on a single A6000) and tested each prediction with a two-sided Wilcoxon signed-rank statistic across the 61 items. Results are reported in `docs/SEPARATRIX_V8A_FINDINGS.md`; the headline numbers are:

- (P1) **confirmed.** Mean cos(tech, bedrock) $= 0.917$ vs.\ mean cos(tech, mystic) $= 0.905$; the predicted ordering holds for $62\%$ of items, $p = 0.031$. The community vectors live in a tight cluster (no discrete prototypes in v8a; the encoder output *is* the community vector), so the absolute separation is small, but the ordering is the predicted one.
- (P3) **confirmed unanimously.** Mean $\hat r$ over prompt tokens $= 1.121$, with $61/61$ prompts above the conservative reference of $\hat r = 0$. Every separatrix prompt is interpretively loaded by the activity gauge; this is the cleanest direct evidence to date for the activity-axis reading of $\hat r$ in §6.5.
- (P4) **falsified, in the strongly opposite direction.** Mean MAH peak on the mystical continuation $= 1.49$ vs.\ mean MAH peak on the bedrock continuation $= 2.07$; the *bedrock* continuation drives larger peak divergence in $95\%$ of items, $p = 3.8 \times 10^{-11}$.

This falsification is the cleanest the SRT-Adapter results have produced, and it is informative about the MAH layer rather than the architecture as a whole. Three readings are compatible with the data; we present them in descending order of plausibility but do not yet adjudicate. (a) *Bedrock is the harder collision.* The mystical branches in this battery are stylistic drift that does not actually commit to an alternate ontology; the bedrock branches do commit to a coordinating philosophical move and require the model to hold two registers in simultaneous play. MAH may be detecting that coordination, not the surface tech-vs-mystic gap. (b) *The mystical branch is in-distribution slop.* Qwen2.5-7B's pretraining includes a great deal of mysticism-as-aesthetic, so the mystical continuation may simply be more predictable in context than the bedrock one, dragging divergence-against-attention down. (c) *Probe-construction artifact.* Bedrock continuations were authored by one hand and may be systematically denser or jargon-heavier than mystical ones; a length- and density-balanced v2 battery is needed to rule this out.

Whichever reading wins, the original §3.3 description of MAH as "divergence quantifying where meaning forks under contextual interpretation" needs to be weakened. The defensible reading the present data supports is that MAH measures *local prediction surprise modulated by community-conditioned attention geometry*; it does not, on this probe, behave as a register-collision detector in the way the earlier text implies. We leave the stronger semantic reading as a hypothesis to test under archetype-conditioned supervision (§5.8 hypothesis (a)) and under the prototype-forcing decode path slated for v9, which would let us run a counterfactual version of the same separatrix sweep (Haylett's prediction (2), not yet testable on v8a). The activity/alignment story (§6.5, §6.8) is unchanged and now has direct probe-level support; the *interpretive-divergence* gloss on MAH is the part of the architectural narrative the data has constrained.

**Independent confirmation from a Takens-based readout.** As an external check on interpretation (a), we ran a Takens delay-embedding head (`srt.modules.thead.TakensHead`, 131K params, no shared parameters with the SRT-Adapter, no training on this probe) on the same separatrix battery, reading the frozen Qwen2.5-7B's hidden states at layer 16 directly. Two findings emerge (full report: `docs/THEAD_SEPARATRIX_V1_FINDINGS.md`). First, the Lyapunov *standard deviation* of the trajectory is markedly larger on mystical continuations than on either technical or bedrock branches (mystical − bedrock $= +0.327$, $p = 2 \times 10^{-9}$, $89\%$ of items): the mystical branch traces a more volatile path through the model's hidden-state phase space, which is the cleanest direct signature of the wider, shallower basin Haylett's separatrix-illusion construct predicts. Second, *recurrence* (orbit periodicity in the delay-coordinate space) is highest on the bedrock branch by a large margin (bedrock − technical $= +3.17$, $p = 10^{-6}$; bedrock − mystical $= +2.77$, $p = 2 \times 10^{-5}$). The bedrock branch produces the most geometrically-organised trajectory of the three. The MAH peak-divergence reading from the SRT-Adapter and the Takens-recurrence reading from the parameter-disjoint T-Head agree on the rank ordering of the three branches: bedrock is the most distinctively-structured, mystical the most volatile, technical sits between. That two heads with no shared parameters and no shared theoretical motivation produce the same rank ordering on the same probe is, we think, a stronger constraint on the underlying picture than either head produces alone, and a direct empirical instantiation of the SRT/Geofinitism convergence sketched in §6.8.

**Depth profile.** Extending the T-Head sweep to four backbone depths (L7, L14, L16, L21) with per-layer eps calibration sharpens both readings further. The Lyapunov-std separation between the mystical and bedrock branches is essentially constant across all four depths ($+0.28$ to $+0.33$, frac+ $\in [0.80, 0.93]$, $p \le 10^{-7}$ everywhere): mystical-as-volatile is a depth-invariant property of the basin the model is in throughout the forward pass, not a late-layer reasoning effect. The recurrence-mean separation between bedrock and technical, by contrast, *builds monotonically with depth*: $-0.41$ (NS) at L7, $+1.36$ ($p = 2 \times 10^{-4}$) at L14, $+3.17$ ($p = 10^{-6}$) at L16, $+3.67$ (frac+ $= 0.85$, $p = 3 \times 10^{-9}$) at L21. The depth at which T-Head recurrence most strongly distinguishes bedrock from technical is the same depth at which the SRT-Adapter's MAH3 layer sits and at which the v1 interiority probe (`docs/INTERIORITY_V1_FINDINGS.md`) reports the largest MAH between-regime spread.

**Per-layer MAH cross-check.** A complementary depth profile from the SRT-Adapter side (`scripts/separatrix_mah_layers.py`, $n = 61$, all three v8a MAH layers) sharpens this further. MAH peak-divergence on the continuation slice ranks the three branches identically at every depth: bedrock $>$ technical $>$ mystical, with bedrock vs technical $p \le 10^{-5}$ everywhere and bedrock vs mystical $p \le 10^{-10}$ everywhere. Bedrock produces the largest local mismatch between the MAH-predicted and observed hidden-state trajectory — i.e. the branch the autoregressive prior fits the worst, which is exactly what one would expect from the most idiosyncratic (non-formulaic) of the three continuations. T-Head recurrence at L21 ranks the branches identically — bedrock $>$ technical $>$ mystical — from a head that shares no parameters and no theoretical motivation with MAH. The two heads disagree on the *depth structure* (MAH is roughly U-shaped with magnitude peaking at L7 and L21; T-Head recurrence rises monotonically and peaks at L21), but they agree on the *rank ordering* of the three branches at every depth where they overlap, and they agree at L21 with effect sizes orders of magnitude above the conventional significance threshold.

**Length-confound test.** In the v1 separatrix battery the technical branches are systematically ~4 words longer than bedrock and mystical; the bedrock-vs-technical MAH effect already runs *opposite* to that length difference (bedrock peak is larger despite being shorter), but the proper falsification is to re-run on a length-matched battery. We built `data/probes/separatrix_illusion_v1_lentrunc.jsonl` by truncating each item's three branches to their per-item minimum word count (mean 13.2 words, range 7-20, no items dropped) and re-ran both heads. The bedrock $>$ technical $>$ mystical MAH-peak rank ordering holds at every depth on the length-matched battery, every contrast at $p \le 10^{-3}$ ($+0.24$/$+0.18$/$+0.31$ for bedrock $-$ tech at L7/L14/L21; $+0.53$/$+0.35$/$+0.49$ for bedrock $-$ mystic, $p \le 10^{-10}$ at every depth). The T-Head Lyap-std mystical-volatility signal holds at every depth, $p \le 10^{-5}$ everywhere; the T-Head recurrence bedrock $>$ technical signal holds at every depth (L21 still the largest at $+3.85$, $p = 2 \times 10^{-9}$). Effect sizes attenuate ~40-50\% under length truncation, which is itself partly a power loss from shorter trajectories, but the convergence claim survives the principal alternative explanation. We treat this triple agreement — same rank ordering, two parameter-disjoint heads, every measured depth, length-confound ruled out — as the strongest current empirical constraint on the SRT/Geofinitism shared-picture claim from §6.8.

**Counterfactual prototype-forcing — a clean negative for Haylett pred (2) on v8a.** The four results above are all *correlational*: they describe what the adapter and the T-Head *read off* the three branches. Haylett's prediction (2) makes a stronger, *causal* claim — that decoding while *forcing* the model into the bedrock (resp. mystical) basin should make the bedrock (resp. mystical) continuation more probable. The v8a forward already supports a `forced_community` override (§5), so we tested this directly: estimate the per-branch centroid of the discovered community vector from the separatrix readouts, then for each item recompute the cross-entropy of each branch's actual continuation tokens under three forcings (force = bedrock centroid, mystical centroid, technical centroid) and compare. Result, $n = 61$ items, Wilcoxon signed-rank: every diagonal effect (CE under the matching forcing $-$ CE under the off-diagonal forcings) is within $\pm 0.002$ nats of zero, every $p > 0.08$. The most forgiving contrast (bedrock branch under the bedrock vs technical forcing) gives $+0.0017$ nats with frac+ $= 0.57$, $p = 0.083$ — not significant uncorrected, certainly not under multiple comparisons. We confirmed the forcing path is wired correctly with a sanity check: forcing a $100\times$-magnified random vector swings CE by $+2.27$ nats (1.34 $\to$ 3.61) on the same item where the natural-magnitude centroid forcing changes CE by $+0.01$ nats. The path is live; the signal isn't there at the magnitude the v8a community vectors produce. Read this as: v8a's MAH heads are *sensitive* to the branch differences (large effect, $p \le 10^{-5}$) but the v8a continuous-community injection path lacks the leverage to *causally drive* decode toward those differences. The three branch centroids sit within $\|\cdot\|_2 \in [0.19, 0.35]$ of each other in 64-d space, with norms $\approx 1$, and the decode head appears to be near-invariant to differences of that magnitude. Pred (2) is therefore *provisionally falsified for v8a* and queued as a primary motivation for v9 (discrete prototypes with construction-enforced inter-cluster spread; full prototype-forcing decode path). The honest reading of §6.9 as a whole, then, is: the *diagnostic* half of the SRT/Geofinitism convergence (read-off of basin structure) is robust under two heads, four depths, and length-matching; the *causal* half (forced-prototype decode) is pending v9 and currently has a clean null on v8a.
...
The v12–v22c lineage (§5.14) closes the gap to standard sentence-embedding evaluation without disturbing any of the v5–v8 interpretability readouts. Holding the v8a scaffold frozen at evaluation and tuning the encoder output channel only, mean MTEB-STS Spearman over 40 splits moves from v8a's $0.210$ to v15a's $0.3634$ (one InfoNCE epoch on NLI, shipped publicly as `RiverRider/srt-adapter-v1.0`, §5.15) to v18's $0.3707$ (CoSENT pairwise loss on $5{,}000$ STSB pairs) and finally to v22c\_a050's $0.3744$ (linear weight average of v18 and a teacher-distilled multilingual sibling, $24/40$ wins over v18, zero additional training). Two methodological lessons are worth emphasising: when the eval metric is rank-based (Spearman), use a rank-aligned loss rather than absolute-value regression on cosines; and when two specialised adapters exist in a connected loss basin, parameter-space interpolation is the cheapest first move before further data-recipe iteration. The architectural interpretation is that the encoder output and the BEN/MAH/RRM stack measure orthogonal axes (alignment $\times$ activity in the §6.5/§6.8 framing): tuning the alignment axis on STS pair data does not perturb the activity axis the BEN reads, and the diagnostic readouts of §§5.4–5.5 and §5.13 hold across the entire v12–v22c\_a050 sweep.

**Availability and release boundary.** The architecture (`srt/`), evaluation harness, examples, and this preprint are public under Apache-2.0 at `https://github.com/space-bacon/SRT`. Adapter weights for v8a, v15a (`RiverRider/srt-adapter-v1.0`), and v22c\_a050 are public on HuggingFace and reproduce the cited STS, regime, and community numbers under the included `scripts/benchmark.py` and `scripts/instrument_eval.py`. The Reddit Discourse Corpus, the per-token annotation pipeline, the C1 scholarly corpus targeted for v9, the teacher-distillation labelling code, and the next-generation training recipe are *not* part of the release; hosted inference and custom-backbone integration are available separately. The intent is to make every numerical claim in this paper independently verifiable from the public artifacts, while keeping the data and recipe IP that supports future generations under the author's control.
...
Su, J. (2022). CoSENT: A more efficient sentence-vector scheme than Sentence-BERT. Spaces.AC.CN technical note. https://kexue.fm/archives/8847
...
| v8a interiority (post-release) | Eight diagnostic probes on the v8a checkpoint (§5.13): layer-as-organ map, community 1-NN, BOS-register length scaling, BOS-sink ablation per channel $\times$ regime, within-prompt trajectories, regime calibration replication, cross-backbone raw-hidden probe (§5.12) | Confirms v8a inject channels function as a fixed-size BOS register (peak-to-peak $\leq 4.8\%$ across $8\times$ length sweep); regime classifier reads content-borne signal; calibration ECE replicates at $9.1\times 10^{-4}$ on 351K tokens; community 1-NN at $7.7\times$ chance; cross-backbone signal latent in Qwen 3-8B and Mistral-7B at comparable strength |
| v12 | Warm-start v8a; one InfoNCE epoch on NLI $+$ Quora $+$ MS MARCO ($396$K pairs); encoder-output channel tuned, interpretability heads frozen at eval | Mean MTEB-STS Spearman $0.346$ ($+0.136$ over v8a); HUMESICK-R $0.792$; first leaderboard-credible STS result |
| v15a ($=$ v1.0) | NLI-only InfoNCE on $\sim 280$K pairs (MS MARCO removed); shipped as `RiverRider/srt-adapter-v1.0` | Mean STS $0.3634$ ($+0.017$ over v12); HUMESICK-R $0.7973$; STSBenchmark $0.4434$; Cap-A NMI $0.5391 \pm 0.008$ ($+0.096$ over v8a, $\approx 13\sigma$); hallucination AUROC $0.576$ retained |
| v17 | $+5{,}000$ STSB pairs trained under MSE on absolute cosine score | Mean STS $0.3531$ ($-0.010$ vs.\ v15a); $0.13$ in-distribution generalisation gap; Pearson-aligned loss on Spearman-scored eval, negative result |
| v18 | Same $5{,}000$ STSB pairs under CoSENT pairwise rank loss (scale $20$, lr $1\times 10^{-5}$, $5$ epochs) | Mean STS $0.3707$ ($+0.0074$ vs.\ v15a, $29/40$ wins); cross-lingual gains despite English-only training; SOTA \#1 |
| v19a, v19b | Shorter v18 schedule; CoSENT on NLI-as-pairs $+$ Quora ($\sim 207$K) | Mean STS $0.3638$ / $0.3709$, both within seed variance of v18 (tie) |
| v20 | CoSENT on NLI categorical labels treated as graded scores (entail $1.0$, neutral $0.5$, contradict $0.0$, $200$K pairs) | Mean STS $0.3667$ ($-0.004$ vs.\ v18); English splits regress (HUMESTSBenchmark $-0.117$); negative result on label scheme |
| v21a | CoSENT on $200$K NLI pairs $+$ $7$K STSB, labels = `mxbai-embed-large-v1` teacher cosines (continuous) | Mean STS $0.3720$ ($+0.0012$ vs.\ v18); IndicCrosslingualSTS $+0.15$ to $+0.22$ on en-bn/tr/ta/te; English regresses ($-0.07$ to $-0.11$); SOTA \#2 (multilingual-leaning) |
| v21b\_a050 / a070 | Linear weight-average of v18 and v20, $\alpha = 0.50 / 0.70$ | Mean STS $0.3709$ / $0.3738$; balanced but inferior to v22c |
| v22a | STSB $5\times$ upsample to increase graded-pair fraction in CoSENT pool | Mean STS $0.3712$ ($+0.0005$ vs.\ v18); within seed variance |
| v22b | Multi-teacher distillation: averaged cosines from `mxbai` and `intfloat/multilingual-e5-large` | Mean STS $0.3708$; ensemble teacher does not improve over single mxbai |
| v22c\_a050 | Linear weight-average of v18 and v21a, $\alpha = 0.50$, zero additional training | Mean STS $0.3744$ ($+0.0037$ vs.\ v18, $24/40$ wins); preserves English (HUMESICK-R $0.7682$); recovers multilingual (STS17 zh-en $0.6647$, $+0.143$ vs.\ v18); current SOTA |
| v9 (in progress) | Closed-loop training target for the inject-back arm (Sections 2.5, 6.3); coverage loss penalizing register concentration (§5.13, §6.3); archetype-conditioned direct supervision (§5.8 hypothesis (a)); adapter re-train on Mistral-7B-v0.3 (§6.6) | TBD |
