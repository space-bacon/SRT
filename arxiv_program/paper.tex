...
(CUDA/bf16 datacenter to Apple-Silicon/MLX-Q4, 100\% head-space
...
measurably (mean centered cosine 0.805 against the datacenter states of
identical strings, with outlier rows). But \textbf{through the head},
local states retrieve their datacenter twins at 98.4\% R@1 unchanged and
\textbf{100.0\% R@1 after the 42KB mean recalibration} (100\% R@5 in
both configurations, pool of 1,000). The head projects away exactly the
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Figure 4. Precision and hardware invariance. Left: the bf16-trained head applied unchanged to 4-bit states loses 0.011 R@1; a 42KB mean recalibration recovers half. Right: across a simultaneous change of hardware, kernels, and quantizer, raw states drift (centered cosine 0.805) but the head projects the drift away, reaching 100\% agreement with the datacenter reference after mean recalibration.}]{figs/fig3_invariance.png}}
...
agreement with the datacenter reference after mean recalibration.}
\end{figure}
...
100\% head-space agreement after 42KB recalibration \\
\end{longtable}
}
