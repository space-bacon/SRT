...
hundreds of image-token positions averages away the subspace structure
that dominates single-token text drift and leaves a clean mean shift,
the mirror image of the text-side geometry. On the end task,
...
recalibration. The recalibration is not optional there: with the shipped
training mean instead of the local one, R@1 drops to 0.401. The claim's
``at most a 42KB recalibration'' clause does real work in both drift
geometries
(\texttt{artifacts/nla/q4/image\_head\_space\_validation\_20260805.json}).
...
pinned to the high-variance subspace the head discards; images: 100\%
agreement via the 42KB recal, i2t R@1 0.640 vs 0.670 datacenter \\
\end{longtable}
}
