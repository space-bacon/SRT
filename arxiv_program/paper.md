...
abstract: |
  ...
  datacenter reference). Deployment tiers, from Raspberry-Pi-class edge
...
data never reaches it. Shuffled-pairs control at the floor.](figs/fig1_ladder.png)
...
variance on identical training pairs.](figs/fig2_scaling.png)
...
(swap splits at 0.50 to 0.57, on the un-normalised captions this
section reports throughout; §6.4 records what stripping the
benchmark's provenance formatting does to them), which is at first
...
in), settles the question, but only once a formatting artifact is taken
out of it. SugarCrepe's positives are human COCO captions and its
foils are model-generated, and the generator punctuates: the foil is
the one ending in a period in 1,384 of the 1,384 pairs where the two
differ, with no counter-example in 7,511. The cached text states were
keyed by the raw caption, so that signature sat inside every text
encoding we scored. Re-encoding both candidates with the trailing
period stripped and the first character lowercased, raw and
normalised on one machine in one session so the re-encode floor
divides out, changes the table. Un-normalised clean five-split macro:
...
Normalised: 0.746, 0.755, 0.746, 0.750. Every cell gains 5 to 9
points, and the asymmetry the un-normalised table showed (text 2.9 to
3.9 points, image 0.4 to 1.4) does not survive it: swapping either
tower now moves the macro by under a point. The artifact was not
neutral across towers, and reading its removal as a tower property is
the error recorded in ledger entry 10. Continued
...
0.658 on the clean five splits with no image, no head, and no
encoder, so every split is reported here as margin over that prior.
That prior needed correcting for the same artifact: tokenising on
whitespace keeps `water.` and `water` apart, so the trailing period
rode inside its final bigram. It scored 0.663, and on swap_obj
exactly 140/245, which is the number a rule reading only punctuation
and capitalisation obtains. Stripping punctuation puts it at 0.658
and swap_obj at 0.543.
...
corrected floor all four cells clear it by similar amounts (+0.088 to
+0.097 on the clean five), where the contaminated pairing had the
gemma-text cells clearing it (+0.044 and +0.031) while the qwen-text
cells sat on it (+0.006 and +0.001). The baseline itself carries
error that point margins conceal. Two independent rebuilds of the prior agree within ±0.02 per
...
and normalisation retires the inversion itself: with the formatting
stripped the real images beat the ablation in all four cells, by
+0.100 and +0.098 in the gemma-text cells and +0.125 and +0.118 in
the qwen-text cells. The inversion existed only in the contaminated
column, and only under the text tower that carried more of the
contamination.
...
ablation's 0.71 to 0.72). That last arm is reported as tied rather
than as evidence: on a swap split the foil is a reordering of the
positive, so both regressors are equal by construction on 64.5% of
swap_obj and 59.6% of swap_att, and the regression has no variance to
fit there. The add splits are the only place it was asked a question.
What survives from the other two arms is that the ablation prices a
learned generic-image direction in text space along which the
benchmark's human captions outscore its generated foils. That is a
property of the text head and the foil construction, not of image
content, and it
...
image-to-caption pairing deranged. On normalised text, averaged over
20 derangements, the clean-five macro falls from 0.755 to 0.570 in
the mixed cell and from 0.750 to 0.564 in the matched cell, so image
...
contaminated column the same margins read +6.6 and +7.0. The margin
is concentrated rather than distributed, but it is not confined: on
the matched cell it is +34.7 on replace_obj, +21.0 on replace_rel and
+17.7 on replace_att, against +10.2 on swap_obj and +9.6 on swap_att.
The earlier reading, that image identity is worth nothing on the
arrangement splits (−0.2 and −1.4), was the formatting. Object
identity still dominates by roughly three to one, which is the
division of labor the inventory measurements below report, reached
from an independent direction, but arrangement is no longer zero and
the papers no longer say it is
...
splits about ten points above their own deranged control. Three
direct measurements support the weaker characterization, and none of
them passes through a caption comparison, so none is touched by the
formatting. Scoring 80 COCO category prompts against 1,560
...
is a property of the frozen pair, with arrangement read markedly less
well than inventory across every combination of towers, objectives,
and decompositions tested. The earlier claim that the text side
dominates the gap, and that the arrangement floor is immovable, both
rested on the un-normalised column
...
ceiling.](figs/fig3_invariance.png)
...
8. **The wall was attributed to the wrong tower.** Five levers
   ...
   second backbone reversed the attribution: the text tower appeared
   to set the band roughly 3.5x more strongly than the image tower.
   Entry 10 supersedes that figure, which was measuring the caption
   formatting rather than the tower; what survives is that an
   elimination inside one pair cannot attribute a limit to either
   side of the pair. The
...
10. **A benchmark's formatting was inside three of our conclusions.**
   SugarCrepe pairs a human COCO caption with a model-generated foil,
   and the generator punctuates: across 7,511 pairs the foil is the
   dotted one in 1,384 of the 1,384 cases where the two differ, with
   no counter-example. Because text states were cached under the raw
   caption, that provenance signature was inside every encoding we
   scored, and it was not neutral. Re-encoding with the trailing
   period stripped, raw and normalised in one session so the
   re-encode floor divides out at 0.0042 on any arm, reverses three
   published claims. Both swap splits move from at or below their
   deranged control to about ten points above it, so "image identity
   is worth nothing on the arrangement splits" was formatting. The
   mean-image ablation stops outscoring real images in every cell, so
   the round-13 inversion was formatting. And the text-tower
   asymmetry inverts, so "the wall is set roughly 3.5x more strongly
   by the text representation" was measuring which tower had absorbed
   more of the artifact. Our own blind prior was reading the same
   tell, scoring exactly 140/245 on swap_obj, the number a
   punctuation-only rule obtains. The lesson is narrower than "check
   your nulls": a benchmark that pairs human text against generated
   text carries a provenance channel that any text encoder can read,
   and normalising it belongs in the harness rather than the data,
   because the comparable column is the contaminated one (§6.4,
   `scripts/sugarcrepe_encode_norm.py`,
   `artifacts/nla/q4/controls_norm_summary.json`).
...
**Retraction (diachronic result).** Earlier versions reported that
...
hypothesis concerns word *classes*, so the unit of analysis must be the
word. Re-run at the word level (41 words) the difference-in-differences
is +0.0011 at permutation p = 0.90: the effect shrinks 43-fold and the
significance disappears. The original figure was pseudoreplication. We
