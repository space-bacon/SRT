...
abstract: |
  ...
  kernels (CUDA/bf16 datacenter to Apple-Silicon/MLX-Q4, 96.7%
  head-space retrieval agreement over the full 5,000-caption pool,
  read against a 99.6% same-runtime reproducibility ceiling). Deployment tiers, from Raspberry-Pi-class edge
...
pool, both directions, four arms, with the reference re-encoded fresh
on CUDA to establish a reproducibility ceiling
...
nothing (93.1%). **Through the head**, agreement rises to 96.4%, and
to **96.7% with the 42KB per-runtime mean recalibration**. The head
projects away much of the subspace in which the drift lives: it
learned a stable core of the representation, not its numerically
fragile surface. The drift is not a mean shift; it is structure the
...
the head applied, and replace the earlier figures. The capability
claim for this tier is scoped to text-side self-retrieval until the
image-side encode is wired on the MLX runtime.
...
R@1 over the full 5,000 pool, mean-centering adds nothing, and the
head lifts agreement to 96.7%, against a 99.6% same-runtime
reproducibility ceiling.](figs/fig3_invariance.png)
...
| hardware / runtime / quantizer | CUDA + bnb → Apple Silicon + MLX | 96.7% head-space agreement (n=5,000) vs a 99.6% same-runtime ceiling; head +3.7 over raw, centering inert |
