...
def fig3_invariance(out: Path) -> None:
    ...
    # Full 5,000-caption pool, cross-runtime self-retrieval R@1, dup-aware
    # (artifacts/nla/q4/head_space_validation_v2_20260805.json)
    labels2 = ["raw", "centered", "PCA\ntop-1024", "random\n1024",
               "head\nshared mu", "head\n+ recal"]
    v2 = [0.933, 0.934, 0.922, 0.932, 0.968, 0.970]
    bars2 = ax2.bar(range(6), v2, 0.6,
                    color=[GREY, GREY, GREY, GREY, VIOLET_LT, VIOLET])
    for b, v in zip(bars2, v2):
        ax2.text(b.get_x() + b.get_width() / 2, v + 0.004, f"{v:.3f}",
                 ha="center", fontsize=6.5, color=INK)
    ax2.axhline(0.9996, color=INK, lw=0.8, ls="--", alpha=0.6)
    ax2.text(5.4, 1.001, "same-runtime ceiling 0.9996", ha="right",
             fontsize=6.5, color=INK, alpha=0.8)
    ax2.set_xticks(range(6), labels2, fontsize=6.5)
    ax2.set_ylim(0.88, 1.02)
    ax2.set_ylabel("cross-runtime R@1 (n=5,000)")
    ax2.set_title("Hardware: CUDA/bf16 → Apple/MLX-Q4;\nonly the head rescues",
